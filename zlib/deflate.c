...
local block_state deflate_max	 OF((deflate_state *s, int flush));
#endif
...
/* 9+*/ {32, 258, 258, 8192, deflate_max}};	// hack for even bettere compression
#endif
...
local block_state deflate_max(s, flush)
    deflate_state *s;
    int flush;
{
    IPos hash_head;       /* head of the hash chain */
    ...
    for (;;) {
        ...
        if (hash_head != NIL && s->strstart - hash_head <= MAX_DIST(s)) {
            ...
            s->match_length = longest_match (s, hash_head);
            /* longest_match() sets match_start */
            if ((s->match_length == MIN_MATCH) && ((s->strstart-s->match_start) > (1 << 14)))
			{
        ...
        */
        if (s->match_length >= MIN_MATCH)
		{
          ...
          for (i = 0; i < 2; i++)
		  {
            ...
            if (((uInt)next_len > (uInt)((s->match_length + 1) + (uInt)i)) 
              || (((uInt)next_len > (uInt)(s->match_length + i)) && ((uInt)(next_dist >> 3) < (uInt)dist)))
			{

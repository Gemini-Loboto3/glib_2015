...
int GetImageType(BYTE *magic)
{
	...
	else if(memcmp(magic+1,"PNG",3)==0) return IMAGE_PNG;
...
static void png_write_data_mem(png_structp png_ptr, png_bytep data, png_size_t length)
{
	MEM_STREAM *pngData=(MEM_STREAM*)png_get_io_ptr(png_ptr);

	if(pngData)
		MemStreamWrite(pngData,data,(int)length);
}

bool write_png(Image &img, LPCTSTR filename)
{
	...
	if (setjmp(png_jmpbuf(png)))
	{
		...
		MemStreamFlush(&str,filename);
		MemStreamClose(&str);
		png_destroy_write_struct(&png,(png_infopp)NULL);
		return 0;
	}

	png_set_write_fn(png,(void*)&str,png_write_data_mem,png_flush);

	png_colorp palette=NULL;
	png_set_IHDR(png, info, img.width, img.height, 8, img.depth <= 8 ? PNG_COLOR_TYPE_PALETTE : PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
	...
	case 4:
	case 8:
		palette = new png_color[1<<img.depth];
		if(!palette)
		{
			png_destroy_write_struct(&png, &info);
			return false;
		...
		for(int i=0; i<1<<img.depth; i++)
		{
			palette[i].red=img.palette[i].rgbRed;
			palette[i].green=img.palette[i].rgbGreen;
			palette[i].blue=img.palette[i].rgbBlue;
		}

		png_set_PLTE(png, info, palette, 1<<img.depth/*PNG_MAX_PALETTE_LENGTH*/);
	}

	png_write_info(png, info);
	png_set_packing(png);

	u8 *row=new u8[img.row_w];
	for(int y=0; y<img.height; y++)
	{
		...
		for(int x=0; x<img.width; )
		{
			...
			{
			case 4:
				*rp++=(img.GetPixelAt(x,y)) | (img.GetPixelAt(x+1,y)<<4);
				x+=2;
				break;
			case 8:
				*rp++=img.GetPixelAt(x,y);
				x++;
				break;
			case 24:
				u32 p=img.GetPixelAt(x,y);
				*rp++=p>>16;
				*rp++=p>>8;
				*rp++=p;
				x++;
...
void* Png::LoadPng(LPCTSTR filename)
{
	BYTE *buffer;
	// bufferization crap
	int size=StoreFile(filename,buffer);
	// return on failure
	if(!size) return 0;
	// do the actual png work
	void *png=LoadPng(buffer,size);
	// deallocate and return
	delete[] buffer;
	return png;
}

void* Png::LoadPng(BYTE *buffer, int size)
{
	png_structp png_ptr;
	png_infop info_ptr;
	PngData data;
	...
	if(!ReadPngInfo(png_ptr,info_ptr)) return 0;
...
//	png_set_packing(png_ptr);
	...
	case PNG_COLOR_TYPE_PALETTE:
		...
		{
			palette[i].rgbRed=pal[i].red;
			palette[i].rgbGreen=pal[i].green;
			palette[i].rgbBlue=pal[i].blue;
		}
		break;
	}
	int _w=PadWidth(width,depth);	// bytes per row
	if(png_get_valid(png_ptr,info_ptr,PNG_INFO_tRNS)) png_set_tRNS_to_alpha(png_ptr);
	png_set_filler(png_ptr, 0xff, PNG_FILLER_AFTER);

	// allocate image
	if(image) delete[] image;
	image=new BYTE[_w*height];
	// allocate line
	//BYTE *line=new BYTE[_w];
	// copy decompressed image
	for(int y=0; y<height; y++)
	{
		//png_read_row(png_ptr,line,NULL);
		png_read_row(png_ptr,&image[y*_w],NULL);
		//CopyMemory(image+(y*_w),line,_w);
	}
...
int Png::ReadPngInfo(png_structp &png_ptr, png_infop &info_ptr)
{
...
#ifndef NO_LIBPNG
void* Image::CreateFromPng(Png *png)
{
	// copy specs
	width=png->width;
	height=png->height;
	/*if(png->depth==32) depth=24;
	else*/ depth=png->depth;
	// bmp actual bytes per row
	int _w=row_w=PadWidth(width,depth);
	// determine bmp image size
	int size=height*_w;
	...
	if(depth<=8) CopyMemory(palette,png->palette,256*sizeof(RGBQUAD));
	...
	case 4:
	case 8:
		for(int y=0; y<height; y++, row+=_w) CopyMemory(rows[y],row,_w);
		break;
	case 24:
		for(int y=0; y<height; y++)
		{
			for(int x=0; x<png->width; x++, row+=3)
			{
				rows[y][x*3]=row[2];
				rows[y][x*3+1]=row[1];
				rows[y][x*3+2]=row[0];
	...
	case 32:
		for(int y=0; y<height; y++)
		{
			for(int x=0; x<png->width; x++, row+=4)
			{
				rows[y][x*4]=row[2];
				rows[y][x*4+1]=row[1];
				rows[y][x*4+2]=row[0];
...
void* Image::LoadFromFile(LPCTSTR name)
{
	...
	case IMAGE_TIM:
		...
		break;
	case IMAGE_PNG:
		if(!png.LoadPng(name)) return 0;
		data=CreateFromPng(&png);
	}
	return data;
}

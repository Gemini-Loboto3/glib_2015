...
void CMarkup::operator=( const CMarkup& markup )
{
	...
	m_aPos.push_back(markup.m_aPos.at(0));
...
bool CMarkup::SetDoc( LPCTSTR szDoc )
{
	...
	if ( (int)m_aPos.size()/*.GetSize()*/ < nStartSize )
...
{
	if ( m_aPos.size()/*.GetSize()*/ && m_aPos[0].iElemChild )
		return true;
	return false;
}
...
GString CMarkup::GetTagName() const
{
...
int CMarkup::x_GetFreePos()
{
	...
	//
	if ( m_iPosFree == m_aPos.size()/*.GetSize()*/ )
...
GString CMarkup::x_GetToken( const CMarkup::TokenPos& token ) const
{
...
int CMarkup::x_ParseNode( CMarkup::TokenPos& token )
{
	...
	if ( szDoc[token.nL] == '<' )
	{
		...
		if ( szEndOfNode )
		{
			...
			token.nNext = (int)((pEnd - szDoc) + _tcslen(szEndOfNode));
		}
	}
...
GString CMarkup::x_GetTagName( int iPos ) const
{
...
GString CMarkup::x_GetAttrib( int iPos, LPCTSTR szAttrib ) const
{
...
GString CMarkup::x_GetData( int iPos ) const
{
...
GString CMarkup::x_TextToDoc( LPCTSTR szText, bool bAttrib ) const
{
...
GString CMarkup::x_TextFromDoc( int nLeft, int nRight ) const
{
...
void CMarkup::x_DocChange( int nLeft, int nReplace, const GString& csInsert )
{
...
bool CMarkup::x_AddElem( LPCTSTR szName, LPCTSTR szValue, bool bInsert, bool bAddChild )
{
	...
	{
		// <NAME>value</NAME>
		GString csValue = x_TextToDoc( szValue );
	...
	if ( bEmptyParent )
	{
		GString csParentTagName = x_GetTagName(iPosParent);
		GString csFormat;
		csFormat = _T(">\r\n");
		csFormat += csInsert;
		csFormat += _T("</");

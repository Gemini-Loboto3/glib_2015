...
public:
	...
	void* LoadPng(BYTE *buffer, int size);

	RGBQUAD palette[256];
	BYTE *image;
	int width, height, depth, color_type, palsize;

protected:
	int ReadPngInfo(png_structp &png_ptr, png_infop &info_ptr);
	int ReadPngImage(FILE *file);
};
#endif // NO_LIBPNG
...
GLIB_API RGBQUAD B5G5R5ToRgb(USHORT clut, u32 transparent=0xFF00FF);
